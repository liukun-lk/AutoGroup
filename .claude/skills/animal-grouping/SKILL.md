...
4. 组数 ≥3 且方差不齐时，Rust 的 Dunnett's T3 缺多重性校正，P 值只有真值的 0.2~0.4 倍，会把本来合格的方案判为不合格。用 Python `verify` 复核，`references/statistics.md` 有量化数据。
...
- **动物分配本身**：由 Levene + 主检验决定排序，这两者 Rust 与精确实现完全一致，所以选出来的划分是可信的（事后检验的偏差不参与排序，不会换掉获胜方案）。
- **要写进报告的数字**：k = 2 直接用 Rust 输出；k ≥ 3 的事后比较 P 值要用 `verify --compare` 的精确值替换，Rust 那一列在小样本下会整片饱和成 `1.000000`（3 组每组 3 只时实测 21 个比较全是 1.0），审阅人一眼就会认为事后检验没在工作。
- **交付文件的暴露面**：`exporter.rs` 的「统计结果」表只有指标名、Levene P、差异检验 P、检验方法、是否达标五列，**不含事后比较**。所以错误的事后 P 值不会进导出的 xlsx，只出现在界面和结果 JSON 里——结论通常是"文件可用、界面上的事后数字要替换"，而不是"重新导出"。
